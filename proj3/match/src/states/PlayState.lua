...
function PlayState:enter(params)
    ...
    self.scoreGoal = self.level * 1.25 * 100 * 3 -- TODO: used a small goal for testing
end
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                -- first make back-end swaps inside data structures
                -- swap grid positions of tiles inside Tile objects
                local tempX = self.highlightedTile.gridX
                local tempY = self.highlightedTile.gridY

                self.highlightedTile.gridX = x
                self.highlightedTile.gridY = y
                ...
                self.board.tiles[tempY][tempX] = newTile

                -- distro code for above swap, but i think it's unnecessarily complicated
                -- self.board.tiles[self.highlightedTile.gridY][self.highlightedTile.gridX] =
                --     self.highlightedTile
                -- self.board.tiles[newTile.gridY][newTile.gridX] = newTile
                
                -- if the swap results in a match, display the swap visually and handle matches
                if self.board:calculateMatches() then

                    -- now tween coordinates between the two tiles so they swap visually (front-end)
                    Timer.tween(0.1, {
                    ...
                    :finish(function()
                        ...
                        -- whenever a swap is made, board changes, so we need to check if valid again
                        -- TODO: not sure if it is working properly
                        -- TODO: make this look nicer visually with tweening and text displays
                        if not self.board:isValidBoard() then
                            self.board:initializeTiles()
                        end
                    end)

                -- if there is no match after swapping, revert swap
                else
                    -- idk if i managed to get this correct
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        -- add score & time for each match
        for k, match in pairs(matches) do
            for j, matchTile in pairs(match) do
                self.score = self.score + matchTile.variety * 50
            end

            -- one more second for each tile matched
